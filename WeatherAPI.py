...
def get_coords(location, country):
    api_key = "62c31e7df36a63f3d0dce16c2291be3a"

    # first get the coordinates of the city
    location_data = requests.get(
        f"http://api.openweathermap.org/geo/1.0/direct?q={location},{country}&limit=3&appid={api_key}")
...
def get_weather_data(location, country):
    ...
    coords = get_coords(location,country)
    ...
    with open(f'weather_data_{location}_{country}.json', 'w') as f:
        json.dump(weather_data, f)

    print(f"------DATA UPDATED ------\n on {time.time()}")

#check if weatherdata.txt has been created, if not, create and update it.
def json_file_update(city, nation):
    try:
        ...
        with open(f'weather_data_{city}_{nation}.json') as file:
            ...
            if current_unix_time - last_update_time >= 43200:
...
def show_weather_data(city = "hamilton", nation = "CA", time = int(time.time())):
    #updates json data first
    json_file_update(city,nation)
